...
edged = cv2.Canny(blur, 3, 50) #이거는 더 조정해도 별 의미 없ㅂㅂ을것ㅂ 같고
...
ww = int(index%len(A)) # B좌표 중 몇 번째 인지 (0~4 중) 얘는 왜 나머지?
# qq = (len(A)-1)-int(math.trunc(float(index/len(A)))) # qq는 A 좌표 중 몇 번째 인지(0~4 중) 얘는 왜 나누고
# ww = (len(B)-1)-int(index%len(A)) # B좌표 중 몇 번째 인지 (0~4 중) 얘는 왜 나머지?
...
#     print(*w, file=file)
#     file.close()
sys.stdout = open('C:/Users/user/Desktop/out.txt', 'w')
print()
for i in range(closed.shape[0]):
    for j in range(closed.shape[1]):
